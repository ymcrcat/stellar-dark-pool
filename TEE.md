...
- Provide HTTPS for users (self-signed certificates), with attestation as the security anchor
- Support graceful degradation outside TEE (local development) without breaking core functionality

## Trust Model

Trust is established through **TEE attestation**, which provides cryptographic proof of:
- Code integrity (compose-hash and RTMR values)
- Hardware authenticity (Intel TDX signature)
- Key ownership (TLS and Stellar public keys bound to TEE)

Self-signed TLS certificates are used for HTTPS connectivity. The certificate fingerprint is informational only; trust is derived from the SPKI hash in the attestation quote.
...
**Network**: Port `443` for HTTPS

**File system**: Writable path for TLS keys and certificates (inside the container). No host mounts for TLS keys or certs.
...
**Endpoints**:
- `GET /attestation` - Returns quote, event log, vm config, reportData, and identity metadata. Supports optional challenge for freshness (hex, <= 64 bytes).
- `GET /info` - Returns TCB info, compose configuration, and metadata.
...
### 1. Remote Attestation Verification

```bash
python3 scripts/verify_remote_attestation.py <base_url> [--save-cert PATH] [--challenge]
```

Verifies compose-hash and TLS certificate match. Optionally saves certificate for pinning.
...
- Optionally saves TLS certificate in PEM format

**Using saved certificate for pinning**:
```bash
...
Public key pinning provides the strongest security guarantee - even if the certificate is reissued, the pinned public key from the TEE attestation must match.
...
Computes compose-hash from a local app-compose.json file. Use cases: pre-deployment verification, audit trail comparison, debug hash mismatches.
...
Complete end-to-end demonstration: verifies TEE attestation, creates test accounts, deposits funds, submits matching orders, and verifies on-chain settlement.
...
**Note**: Deploy the contract first, then deploy the matching engine configured with that contract ID.
...
The `dcap-qvl` verifier checks Intel's cryptographic signature, certificate chain back to Intel's root CA, TCB status, and measurement registers (MRTD, RTMR0-3, reportData). This provides local verification without relying on third-party APIs.
...
- **Attestation availability**: Only available inside Phala Cloud; local dev should return 503 with a clear message
- **Quote caching**: Quote generation should be cached (short TTL) to reduce overhead
- **Key ephemerality**: TLS keys are ephemeral per deployment; clients should re-verify after restart
- **Image digest pinning**: Use immutable image digests (SHA256) in deployment for reproducible measurements and security
...
- **Environment variables**: Variables like `${SETTLEMENT_CONTRACT_ID}` are resolved at deployment time and baked into the attested compose-hash
